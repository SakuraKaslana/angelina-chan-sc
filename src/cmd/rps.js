    ...
    alias: ["rockpaperscissor"],
     ...
     .then(() =>{
		message.channel.awaitMessages({ filter, max: 1, time: 20000, errors: ['time'] })
			.then(collected => {
                var valid = true;
				const temp = collected.first();
                var UserChoiceRPS = temp.content.toLowerCase();
                switch(UserChoiceRPS){
                    case UserChoiceRPS = "rock":
                        var UserChoiceN = 1;
                        break;
                ...
                if (valid===false){message.channel.send(`Invalid choice bruh`)} else {
                    var bc = Math.floor(Math.random()*3)+1  
                    switch(bc){
                        case bc = 1:
                            var BotChoice = "Rock";
                    ...
                    if (UserChoiceN === bc){message.channel.send(`It's a draw\nYour selection: ${UserChoiceRPS}\nMy selection: ${BotChoice}`)}
                    //U:Rock B:Paper        win
                    else if (UserChoiceN === 1 && bc === 2){message.channel.send(`<:angie_like:988732747059175444> I win\nYour selection: Rock\nMy selection: ${BotChoice}`)}
                    //U:Rock B:Scissor      lose
                    else if (UserChoiceN === 1 && bc === 3){message.channel.send(`<:ange_angy:988730780031279125> You win\nYour selection: Rock\nMy selection: ${BotChoice}`)}
                    //U:Paper B:Rock        lose
                    else if (UserChoiceN === 2 && bc === 1){message.channel.send(`<:Tsukasa_4koma:988738686541787168> You win\nYour selection: Paper\nMy selection: ${BotChoice}`)}
                    //U:Paper B:Scissor     win
                    else if (UserChoiceN === 2 && bc === 3){message.channel.send(`<:ange_ayaya:956939948634828872> I win\nYour selection: Paper\nMy selection: ${BotChoice}`)}
                    //U:Scissor B:Rock      win
                    else if (UserChoiceN === 3 && bc === 1){message.channel.send(`<:Minori_4koma:988736646952390656> I win\nYour selection: Scissor\nMy selection: ${BotChoice}`)}
                    //U:Scissor B:Paper     lose
                    else if (UserChoiceN === 3 && bc === 2){message.channel.send(`<:Honami_4koma:988736648697233428> You win\nYour selection: Scissor\nMy selection: ${BotChoice}`)}
                    var time = new Date().toLocaleTimeString('en-US', { hour12: false,
                        hour: "numeric",
                        minute: "numeric",
                        second: "numeric"})
			...
			.catch(collected => {
				message.channel.send(`You took too long to choose, aborting the game ;-;`);
			});
	})
                }
